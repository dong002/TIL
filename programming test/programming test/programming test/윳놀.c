...
int CNT = 0, rvic = 0, bvic = 0,cntemp=1,redmal=0,bluemal=0;
...
void check(int x, int y, int colors, int temp);
...
int main() {
   ...
   //look(n);
		...
		while (1) {
			...
			if ((yutbord[fy][fx / 2] == 7 || yutbord[fy][fx / 2] == 9 || yutbord[fy][fx / 2] == 11 || yutbord[fy][fx / 2] == 13 && CNT % 2 == 1) {//판위에 있는 말들 움직이는거
				redmal = yutbord[fy][fx / 2];
				/*if (yut == -1) {
					spot;
					Backdo(CX, CY);
				}*/
				//else {
					...
					if (CX == 10 && CY == 5) {
						gotoxy(25, 10);
						printf("고르셈(1은 왼쪽길,2는 오른쪽길) 1or2:");
						scanf_s("%d", &c);
						gotoxy(10, 5);
						switch (c) {
					...
					if (CX == 20 && CY == 0) {
						gotoxy(25, 10);
						printf("고르셈(1은 직선길,2는 대각선길) 1or2:");
						scanf_s("%d", &c);
						gotoxy(20, 0);
						switch (c) {
						case 1: break;
				...
				if (CX != 20 || CY != 10) {
					yutbord[CY][CX / 2] = redmal;
				}	
				//상대말 잡는거는 여따넣을거임 그리고 모,윷등에관한 예외도 여기서처리
				visual(yutbord);//화면 갱신
				break;
			}
			else if (yutbord[CY][CX / 2] == 8 || yutbord[CY][CX / 2] == 10 || yutbord[CY][CX / 2] == 12 || yutbord[CY][CX / 2] == 14 && CNT % 2 == 0) {
			bluemal = yutbord[CY][CX / 2];
				/*if (yut == -1) {
					spot;
					Backdo(CX, CY);
				}*/
				...
				spot;
				check(CX, CY, 1, temp);
				yutbord[CY][CX / 2] = redmal;//이부분은 맨처음 판에 온 말이 움직이는건데 솔직히 없어도 될거같음 근데 이거 없앨라면 저위에있는걸 
